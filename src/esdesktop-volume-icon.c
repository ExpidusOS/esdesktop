...
struct _EsdesktopVolumeIconPrivate
{
    gchar *tooltip;
    gchar *label;
    GVolume *volume;
...
static void esdesktop_volume_icon_finalize(GObject *obj);

static GdkPixbuf *esdesktop_volume_icon_peek_pixbuf(EsdesktopIcon *icon,
                                                    gint width, gint height);
static const gchar *esdesktop_volume_icon_peek_label(EsdesktopIcon *icon);
static gchar *esdesktop_volume_icon_get_identifier(EsdesktopIcon *icon);
static GdkPixbuf *esdesktop_volume_icon_peek_tooltip_pixbuf(EsdesktopIcon *icon,
                                                            gint width, gint height);
static const gchar *esdesktop_volume_icon_peek_tooltip(EsdesktopIcon *icon);
static GdkDragAction esdesktop_volume_icon_get_allowed_drag_actions(EsdesktopIcon *icon);
static GdkDragAction esdesktop_volume_icon_get_allowed_drop_actions(EsdesktopIcon *icon,
                                                                    GdkDragAction *suggested_action);
static gboolean esdesktop_volume_icon_do_drop_dest(EsdesktopIcon *icon,
                                                   EsdesktopIcon *src_icon,
                                                   GdkDragAction action);
static gboolean esdesktop_volume_icon_populate_context_menu(EsdesktopIcon *icon,
                                                            GtkWidget *menu);

static GFileInfo *esdesktop_volume_icon_peek_file_info(EsdesktopFileIcon *icon);
static GFileInfo *esdesktop_volume_icon_peek_filesystem_info(EsdesktopFileIcon *icon);
static GFile *esdesktop_volume_icon_peek_file(EsdesktopFileIcon *icon);
static void esdesktop_volume_icon_update_file_info(EsdesktopFileIcon *icon,
                                                   GFileInfo *info);
static gboolean esdesktop_volume_icon_activated(EsdesktopIcon *icon);
static gboolean volume_icon_changed_timeout(gpointer user_data);
static void esdesktop_volume_icon_changed(GVolume *volume,
                                          EsdesktopVolumeIcon *volume_icon);

#ifdef HAVE_LUNARX
static void esdesktop_volume_icon_tfi_init(LunarxFileInfoIface *iface);

G_DEFINE_TYPE_EXTENDED(EsdesktopVolumeIcon, esdesktop_volume_icon,
                       ESDESKTOP_TYPE_FILE_ICON, 0,
                       G_IMPLEMENT_INTERFACE(LUNARX_TYPE_FILE_INFO,
                                             esdesktop_volume_icon_tfi_init)
                       G_ADD_PRIVATE(EsdesktopVolumeIcon)
                       )
#else
G_DEFINE_TYPE_WITH_PRIVATE(EsdesktopVolumeIcon, esdesktop_volume_icon,
                           ESDESKTOP_TYPE_FILE_ICON)
#endif



static GQuark esdesktop_volume_icon_activated_quark;



static void
esdesktop_volume_icon_class_init(EsdesktopVolumeIconClass *klass)
{
    ...
    esdesktop_volume_icon_activated_quark = g_quark_from_static_string("esdesktop-volume-icon-activated");

    gobject_class->finalize = esdesktop_volume_icon_finalize;

    icon_class->peek_pixbuf = esdesktop_volume_icon_peek_pixbuf;
    icon_class->peek_label = esdesktop_volume_icon_peek_label;
    icon_class->get_identifier = esdesktop_volume_icon_get_identifier;
    icon_class->peek_tooltip_pixbuf = esdesktop_volume_icon_peek_tooltip_pixbuf;
    icon_class->peek_tooltip = esdesktop_volume_icon_peek_tooltip;
    icon_class->get_allowed_drag_actions = esdesktop_volume_icon_get_allowed_drag_actions;
    icon_class->get_allowed_drop_actions = esdesktop_volume_icon_get_allowed_drop_actions;
    icon_class->do_drop_dest = esdesktop_volume_icon_do_drop_dest;
    icon_class->populate_context_menu = esdesktop_volume_icon_populate_context_menu;
    icon_class->activated = esdesktop_volume_icon_activated;

    file_icon_class->peek_file_info = esdesktop_volume_icon_peek_file_info;
    file_icon_class->peek_filesystem_info = esdesktop_volume_icon_peek_filesystem_info;
    file_icon_class->peek_file = esdesktop_volume_icon_peek_file;
    file_icon_class->update_file_info = esdesktop_volume_icon_update_file_info;
...
static void
esdesktop_volume_icon_init(EsdesktopVolumeIcon *icon)
{
    icon->priv = esdesktop_volume_icon_get_instance_private(icon);
}

static void
esdesktop_volume_icon_finalize(GObject *obj)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(obj);
    ...
    G_OBJECT_CLASS(esdesktop_volume_icon_parent_class)->finalize(obj);
}

#ifdef HAVE_LUNARX
static void
esdesktop_volume_icon_tfi_init(LunarxFileInfoIface *iface)
{
...
static gboolean
esdesktop_volume_icon_is_mounted(EsdesktopIcon *icon)
{
    GVolume *volume = NULL;
    GMount *mount = NULL;
    gboolean ret = FALSE;
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);

    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon), FALSE);

    volume = esdesktop_volume_icon_peek_volume(volume_icon);
...
static GIcon *
esdesktop_volume_icon_load_icon(EsdesktopIcon *icon)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
...
static GdkPixbuf *
esdesktop_volume_icon_peek_pixbuf(EsdesktopIcon *icon,
                                  gint width, gint height)
{
    gint opacity = 100;
    GIcon *gicon = NULL;
    ...
    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon), NULL);

    if(!esdesktop_file_icon_has_gicon(ESDESKTOP_FILE_ICON(icon)))
        gicon = esdesktop_volume_icon_load_icon(icon);
    else
        g_object_get(ESDESKTOP_FILE_ICON(icon), "gicon", &gicon, NULL);

    /* If the volume isn't mounted show it as semi-transparent */
    if(!esdesktop_volume_icon_is_mounted(icon))
        opacity = 50;

    pix = esdesktop_file_utils_get_icon(gicon, height, height, opacity);

    return pix;
}

static GdkPixbuf *
esdesktop_volume_icon_peek_tooltip_pixbuf(EsdesktopIcon *icon,
                                          gint width, gint height)
{
    GIcon *gicon = NULL;
...
const gchar *
esdesktop_volume_icon_peek_label(EsdesktopIcon *icon)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
...
static gchar *
esdesktop_volume_icon_get_identifier(EsdesktopIcon *icon)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
    gchar *uuid;
    ...
    if(uuid == NULL)
        return g_strdup(esdesktop_volume_icon_peek_label(icon));

    return uuid;
}

static GdkDragAction
esdesktop_volume_icon_get_allowed_drag_actions(EsdesktopIcon *icon)
{
     ...
     * we should just disallow everything, since, even if its LunarVfsInfo
...
static GdkDragAction
esdesktop_volume_icon_get_allowed_drop_actions(EsdesktopIcon *icon,
                                               GdkDragAction *suggested_action)
{
...
static gboolean
esdesktop_volume_icon_do_drop_dest(EsdesktopIcon *icon,
                                 EsdesktopIcon *src_icon,
                                 GdkDragAction action)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
    ...
    g_return_val_if_fail(esdesktop_volume_icon_get_allowed_drop_actions(icon, NULL),
                         FALSE);
...
static const gchar *
esdesktop_volume_icon_peek_tooltip(EsdesktopIcon *icon)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
    GFileInfo *fs_info = esdesktop_file_icon_peek_filesystem_info(ESDESKTOP_FILE_ICON(icon));
...
static void
esdesktop_volume_icon_eject_finish(GObject *object,
                                   GAsyncResult *result,
                                   gpointer user_data)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(user_data);
    GtkWidget *icon_view = esdesktop_icon_peek_icon_view(ESDESKTOP_ICON(icon));
    ...
    g_return_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon));
    ...
    if(!eject_successful) {
        ...
        if(error->domain != G_IO_ERROR || error->code != G_IO_ERROR_FAILED_HANDLED) {
            ...
            expidus_message_dialog(toplevel ? GTK_WINDOW(toplevel) : NULL,
...
#ifdef HAVE_LIBNOTIFY
    esdesktop_notify_eject_finish(volume, eject_successful);
#endif
...
static void
esdesktop_volume_icon_unmount_finish(GObject *object,
                                     GAsyncResult *result,
                                     gpointer user_data)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(user_data);
...
#ifdef HAVE_LIBNOTIFY
    esdesktop_notify_unmount_finish(mount, unmount_successful);
#endif
...
static void
esdesktop_volume_icon_mount_finish(GObject *object,
                                   GAsyncResult *result,
                                   gpointer user_data)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(user_data);
    ...
    if(!g_volume_mount_finish(volume, result, &error)) {
        ...
        if(file && info) {
            ...
            esdesktop_file_icon_update_file_info(ESDESKTOP_FILE_ICON(icon), info);

            activated = GPOINTER_TO_UINT(g_object_get_qdata(G_OBJECT(icon),
                                                            esdesktop_volume_icon_activated_quark));
            if(activated) {
                EsdesktopIcon *icon_p = ESDESKTOP_ICON(icon);
                ESDESKTOP_ICON_CLASS(esdesktop_volume_icon_parent_class)->activated(icon_p);
            }
            g_object_set_qdata(G_OBJECT(icon), esdesktop_volume_icon_activated_quark, NULL);
        } else {
            if(icon->priv->file)
                g_object_unref(icon->priv->file);
            icon->priv->file = NULL;

            esdesktop_file_icon_update_file_info(ESDESKTOP_FILE_ICON(icon), NULL);
        }
...
static void
esdesktop_volume_icon_menu_mount(GtkWidget *widget, gpointer user_data)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(user_data);
    GtkWidget *icon_view = esdesktop_icon_peek_icon_view(ESDESKTOP_ICON(icon));
    GtkWidget *toplevel = gtk_widget_get_toplevel(icon_view);
    ...
    volume = esdesktop_volume_icon_peek_volume(icon);
    mount = g_volume_get_mount(volume);
    ...
    g_volume_mount(volume, G_MOUNT_MOUNT_NONE, operation, NULL,
                   esdesktop_volume_icon_mount_finish,
                   g_object_ref(icon));
...
static void
esdesktop_volume_icon_menu_unmount(GtkWidget *widget, gpointer user_data)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(user_data);
    GtkWidget *icon_view = esdesktop_icon_peek_icon_view(ESDESKTOP_ICON(icon));
    GtkWidget *toplevel = gtk_widget_get_toplevel(icon_view);
...
#ifdef HAVE_LIBNOTIFY
    esdesktop_notify_unmount(mount);
#endif
    ...
    g_mount_unmount_with_operation(mount,
                                   ...
                                   operation,
                                   NULL,
                                   esdesktop_volume_icon_unmount_finish,
                                   g_object_ref(icon));
...
static void
esdesktop_volume_icon_menu_eject(GtkWidget *widget,
                                 gpointer user_data)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(user_data);
    GtkWidget *icon_view = esdesktop_icon_peek_icon_view(ESDESKTOP_ICON(icon));
    ...
    if(g_volume_can_eject(volume)) {
        ...
        g_volume_eject_with_operation(volume,
                                      ...
                                      operation,
                                      NULL,
                                      esdesktop_volume_icon_eject_finish,
                                      g_object_ref(icon));
    } else {
        /* If we can't eject the volume try to unmount it */
        esdesktop_volume_icon_menu_unmount(widget, user_data);
    }
...
static void
esdesktop_volume_icon_menu_properties(GtkWidget *widget,
                                      gpointer user_data)
{
    EsdesktopFileIcon *icon = ESDESKTOP_FILE_ICON(user_data);
    GFile *file;

    file = esdesktop_file_icon_peek_file(icon);
    esdesktop_file_utils_show_properties_dialog(file,
                                                ESDESKTOP_VOLUME_ICON(icon)->priv->gscreen,
                                                NULL);
}

static void
esdesktop_volume_icon_add_context_menu_option(EsdesktopIcon *icon,
                                              ...
                                              GCallback callback)
{
    ...
    mi = esdesktop_menu_create_menu_item_with_mnemonic(icon_label, img);
    gtk_widget_show(mi);
...
static gboolean
esdesktop_volume_icon_populate_context_menu(EsdesktopIcon *icon,
                                            GtkWidget *menu)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
    ...
    g_signal_connect_swapped(G_OBJECT(mi), "activate",
                             G_CALLBACK(esdesktop_icon_activated), icon);
    ...
    if(mount && g_volume_can_eject(volume)) {
        ...
        esdesktop_volume_icon_add_context_menu_option(icon, icon_name, icon_label,
                        menu, G_CALLBACK(esdesktop_volume_icon_menu_eject));
    }
    ...
    if(mount && g_mount_can_unmount(mount)) {
        ...
        esdesktop_volume_icon_add_context_menu_option(icon, icon_name, icon_label,
                        menu, G_CALLBACK(esdesktop_volume_icon_menu_unmount));
    }
    ...
    if(!mount && g_volume_can_mount(volume)) {
        ...
        esdesktop_volume_icon_add_context_menu_option(icon, icon_name, icon_label,
                        menu, G_CALLBACK(esdesktop_volume_icon_menu_mount));
    }
    ...
    if(!volume_icon->priv->file_info)
        ...
        esdesktop_volume_icon_add_context_menu_option(icon, icon_name, icon_label,
                    menu, G_CALLBACK(esdesktop_volume_icon_menu_properties));
    }
...
static GFileInfo *
esdesktop_volume_icon_peek_file_info(EsdesktopFileIcon *icon)
{
    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon), NULL);
    return ESDESKTOP_VOLUME_ICON(icon)->priv->file_info;
}

static GFileInfo *
esdesktop_volume_icon_peek_filesystem_info(EsdesktopFileIcon *icon)
{
    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon), NULL);
    return ESDESKTOP_VOLUME_ICON(icon)->priv->filesystem_info;
}

static GFile *
esdesktop_volume_icon_peek_file(EsdesktopFileIcon *icon)
{
    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon), NULL);
    return ESDESKTOP_VOLUME_ICON(icon)->priv->file;
}

static void
esdesktop_volume_icon_update_file_info(EsdesktopFileIcon *icon,
                                       GFileInfo *info)
{
    EsdesktopVolumeIcon *volume_icon = ESDESKTOP_VOLUME_ICON(icon);
...
static gboolean
esdesktop_volume_icon_activated(EsdesktopIcon *icon_p)
{
    EsdesktopVolumeIcon *icon = ESDESKTOP_VOLUME_ICON(icon_p);
    GVolume *volume = esdesktop_volume_icon_peek_volume(icon);
    GMount *mount;
    ...
    if(!mount) {
        ...
        g_object_set_qdata(G_OBJECT(icon), esdesktop_volume_icon_activated_quark,
                           GUINT_TO_POINTER(TRUE));

        /* mount the volume and open the folder in the mount finish callback */
        esdesktop_volume_icon_menu_mount(NULL, icon);
        ...
        return ESDESKTOP_ICON_CLASS(esdesktop_volume_icon_parent_class)->activated(icon_p);
    }
}
...
{
    EsdesktopVolumeIcon *volume_icon = user_data;
    GMount *mount;
    gboolean mounted_before = FALSE;
    gboolean mounted_after = FALSE;

    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(volume_icon), FALSE);
    ...
    if(mounted_before != mounted_after) {
        ...
        /* not really easy to check if this changed or not, so just invalidate it */
        esdesktop_icon_invalidate_pixbuf(ESDESKTOP_ICON(volume_icon));
        esdesktop_icon_pixbuf_changed(ESDESKTOP_ICON(volume_icon));
...
static void
esdesktop_volume_icon_changed(GVolume *volume,
                              EsdesktopVolumeIcon *volume_icon)
{
    g_return_if_fail(G_IS_VOLUME(volume));
    g_return_if_fail(ESDESKTOP_IS_VOLUME_ICON(volume_icon));
...
EsdesktopVolumeIcon *
esdesktop_volume_icon_new(GVolume *volume,
                          GdkScreen *screen)
{
    EsdesktopVolumeIcon *volume_icon;
    GMount *mount;
    ...
    volume_icon = g_object_new(ESDESKTOP_TYPE_VOLUME_ICON, NULL);
    ...
    g_signal_connect(volume, "changed",
                     G_CALLBACK(esdesktop_volume_icon_changed),
                     volume_icon);
...
GVolume *
esdesktop_volume_icon_peek_volume(EsdesktopVolumeIcon *icon)
{
    g_return_val_if_fail(ESDESKTOP_IS_VOLUME_ICON(icon), NULL);
