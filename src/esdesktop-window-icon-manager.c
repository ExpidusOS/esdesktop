...
static void esdesktop_window_icon_manager_set_property(GObject *object,
                                                       guint property_id,
                                                       const GValue *value,
                                                       GParamSpec *pspec);
static void esdesktop_window_icon_manager_get_property(GObject *object,
                                                       guint property_id,
                                                       GValue *value,
                                                       GParamSpec *pspec);
static void esdesktop_window_icon_manager_finalize(GObject *obj);
static void esdesktop_window_icon_manager_icon_view_manager_init(EsdesktopIconViewManagerIface *iface);

static gboolean esdesktop_window_icon_manager_real_init(EsdesktopIconViewManager *manager,
                                                        EsdesktopIconView *icon_view);
static void esdesktop_window_icon_manager_fini(EsdesktopIconViewManager *manager);
...
{
    GHashTable *icons;
    EsdesktopWindowIcon *selected_icon;
} EsdesktopWindowIconWorkspace;

struct _EsdesktopWindowIconManagerPrivate
{
    gboolean inited;
    ...
    gint nworkspaces;
    gint active_ws_num;
    EsdesktopWindowIconWorkspace **icon_workspaces;
};


G_DEFINE_TYPE_EXTENDED(EsdesktopWindowIconManager,
                       esdesktop_window_icon_manager,
                       G_TYPE_OBJECT, 0,
                       G_IMPLEMENT_INTERFACE(ESDESKTOP_TYPE_ICON_VIEW_MANAGER,
                                             esdesktop_window_icon_manager_icon_view_manager_init)
                       G_ADD_PRIVATE(EsdesktopWindowIconManager))

static void
esdesktop_window_icon_manager_class_init(EsdesktopWindowIconManagerClass *klass)
{
    GObjectClass *gobject_class = (GObjectClass *)klass;

    gobject_class->set_property = esdesktop_window_icon_manager_set_property;
    gobject_class->get_property = esdesktop_window_icon_manager_get_property;
    gobject_class->finalize = esdesktop_window_icon_manager_finalize;
...
static void
esdesktop_window_icon_manager_init(EsdesktopWindowIconManager *wmanager)
{
    wmanager->priv = esdesktop_window_icon_manager_get_instance_private(wmanager);
}

static void
esdesktop_window_icon_manager_set_property(GObject *object,
                                           guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec)
{
    EsdesktopWindowIconManager *wmanager = ESDESKTOP_WINDOW_ICON_MANAGER(object);
...
static void
esdesktop_window_icon_manager_get_property(GObject *object,
                                           guint property_id,
                                           GValue *value,
                                           GParamSpec *pspec)
{
...
static void
esdesktop_window_icon_manager_finalize(GObject *obj)
{
    EsdesktopWindowIconManager *wmanager = ESDESKTOP_WINDOW_ICON_MANAGER(obj);

    TRACE("entering");

    if(wmanager->priv->inited)
        esdesktop_window_icon_manager_fini(ESDESKTOP_ICON_VIEW_MANAGER(wmanager));

    G_OBJECT_CLASS(esdesktop_window_icon_manager_parent_class)->finalize(obj);
}

static void
esdesktop_window_icon_manager_icon_view_manager_init(EsdesktopIconViewManagerIface *iface)
{
    iface->manager_init = esdesktop_window_icon_manager_real_init;
    iface->manager_fini = esdesktop_window_icon_manager_fini;
}


static void
esdesktop_window_icon_manager_icon_selection_changed_cb(EsdesktopIconView *icon_view,
                                                        gpointer user_data)
{
    EsdesktopWindowIconManager *wmanager = user_data;
    GList *selected;

    selected = esdesktop_icon_view_get_selected_items(icon_view);
    if(selected) {
        EsdesktopWindowIcon *window_icon = ESDESKTOP_WINDOW_ICON(selected->data);
        gint ws = esdesktop_window_icon_get_workspace(window_icon);
...
static EsdesktopWindowIcon *
esdesktop_window_icon_manager_add_icon(EsdesktopWindowIconManager *wmanager,
                                       WnckWindow *window,
                                       gint ws_num)
{
    EsdesktopWindowIcon *icon = esdesktop_window_icon_new(window, ws_num);

    g_hash_table_insert(wmanager->priv->icon_workspaces[ws_num]->icons,
                        window, icon);

    if(ws_num == wmanager->priv->active_ws_num)
        esdesktop_icon_view_add_item(wmanager->priv->icon_view,
                                     ESDESKTOP_ICON(icon));
...
static void
esdesktop_add_window_icons_foreach(gpointer key,
                                   gpointer value,
                                   gpointer user_data)
{
    EsdesktopIcon *icon = value;
    EsdesktopWindowIconManager *wmanager = user_data;
    esdesktop_icon_view_add_item(wmanager->priv->icon_view, icon);
}
...
workspace_changed_cb(WnckScreen *wnck_screen,
                     ...
                     gpointer user_data)
{
    EsdesktopWindowIconManager *wmanager = ESDESKTOP_WINDOW_ICON_MANAGER(user_data);
    gint n;
    WnckWorkspace *ws;
    ...
    esdesktop_icon_view_remove_all(wmanager->priv->icon_view);
    ...
    if(!wmanager->priv->icon_workspaces[n]->icons) {
        ...
        for(l = windows; l; l = l->next) {
            ...
            {
                esdesktop_window_icon_manager_add_icon(wmanager,
                                                       window, n);
            }
        }
    } else
        g_hash_table_foreach(wmanager->priv->icon_workspaces[n]->icons,
                             esdesktop_add_window_icons_foreach, wmanager);

    if(wmanager->priv->icon_workspaces[n]->selected_icon) {
        esdesktop_icon_view_select_item(wmanager->priv->icon_view,
                                        ESDESKTOP_ICON(wmanager->priv->icon_workspaces[n]->selected_icon));
    }
}
                     ...
                     gpointer user_data)
{
    ...
    wmanager->priv->icon_workspaces[ws_num] = g_new0(EsdesktopWindowIconWorkspace,
                                                     1);
}
                        ...
                        gpointer user_data)
{
    ...
    if(is_add) {
        for(; i < max_i; i++) {
            ...
            esdesktop_window_icon_manager_add_icon(wmanager, window, i);
        }
    } else {
        for(; i < max_i; i++) {
            TRACE("loop: %d", i);
            ...
            if(icon) {
                ...
                if(i == wmanager->priv->active_ws_num) {
                    ...
                    esdesktop_icon_view_remove_item(wmanager->priv->icon_view,
                                                    ESDESKTOP_ICON(icon));
...
{
    EsdesktopWindowIconManager *wmanager = data;
...
static void
esdesktop_window_icon_manager_populate_context_menu(ExpidusDesktop *desktop,
                                                    GtkMenuShell *menu,
                                                    gpointer user_data)
{
    EsdesktopWindowIconManager *wmanager = ESDESKTOP_WINDOW_ICON_MANAGER(user_data);
    EsdesktopWindowIconWorkspace *wiws = wmanager->priv->icon_workspaces[wmanager->priv->active_ws_num];

    if(!wiws->selected_icon)
        return;

    esdesktop_icon_populate_context_menu(ESDESKTOP_ICON(wiws->selected_icon),
                                         GTK_WIDGET(menu));
}
...
esdesktop_window_icon_manager_new(GdkScreen *gscreen)
{
    g_return_val_if_fail(GDK_IS_SCREEN(gscreen), NULL);
    return g_object_new(ESDESKTOP_TYPE_WINDOW_ICON_MANAGER,
                        "screen", gscreen,
                        NULL);
}


/* EsdesktopIconViewManager virtual functions */

static gboolean
esdesktop_window_icon_manager_real_init(EsdesktopIconViewManager *manager,
                                        EsdesktopIconView *icon_view)
{
    EsdesktopWindowIconManager *wmanager = ESDESKTOP_WINDOW_ICON_MANAGER(manager);
    GList *windows, *l;
    gint i;

    wmanager->priv->icon_view = icon_view;
    esdesktop_icon_view_set_selection_mode(icon_view, GTK_SELECTION_SINGLE);
    g_signal_connect(G_OBJECT(icon_view), "icon-selection-changed",
                     G_CALLBACK(esdesktop_window_icon_manager_icon_selection_changed_cb),
                     wmanager);
    ...
    g_signal_connect(G_OBJECT(wmanager->priv->desktop), "populate-root-menu",
                     G_CALLBACK(esdesktop_window_icon_manager_populate_context_menu),
                     wmanager);
    ...
    wmanager->priv->icon_workspaces = g_malloc0(wmanager->priv->nworkspaces
                                                * sizeof(gpointer));
    for(i = 0; i < wmanager->priv->nworkspaces; ++i) {
        wmanager->priv->icon_workspaces[i] = g_new0(EsdesktopWindowIconWorkspace,
                                                    1);
    }
...
static void
esdesktop_window_icon_manager_fini(EsdesktopIconViewManager *manager)
{
    EsdesktopWindowIconManager *wmanager = ESDESKTOP_WINDOW_ICON_MANAGER(manager);
    gint i;
    GList *windows, *l;
