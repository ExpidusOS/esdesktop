...
#ifdef ENABLE_DESKTOP_ICONS
    ExpidusDesktopIconStyle icons_style;
...
static void expidus_desktop_finalize(GObject *object);
static void expidus_desktop_set_property(GObject *object,
                                      guint property_id,
                                      const GValue *value,
                                      GParamSpec *pspec);
static void expidus_desktop_get_property(GObject *object,
                                      guint property_id,
                                      GValue *value,
                                      GParamSpec *pspec);

static void expidus_desktop_realize(GtkWidget *widget);
static void expidus_desktop_unrealize(GtkWidget *widget);
static gboolean expidus_desktop_button_press_event(GtkWidget *widget,
                                                GdkEventButton *evt);
static gboolean expidus_desktop_button_release_event(GtkWidget *widget,
                                                  GdkEventButton *evt);
static gboolean expidus_desktop_popup_menu(GtkWidget *widget);

static gboolean expidus_desktop_draw(GtkWidget *w,
                                  cairo_t *cr);
static gboolean expidus_desktop_delete_event(GtkWidget *w,
                                          GdkEventAny *evt);
static void expidus_desktop_style_updated(GtkWidget *w);

static void expidus_desktop_set_single_workspace_mode(ExpidusDesktop *desktop,
                                                   gboolean single_workspace);
static void expidus_desktop_set_single_workspace_number(ExpidusDesktop *desktop,
                                                     gint workspace_num);

static gboolean expidus_desktop_get_single_workspace_mode(ExpidusDesktop *desktop);
static gint expidus_desktop_get_current_workspace(ExpidusDesktop *desktop);

#ifdef ENABLE_DESKTOP_ICONS
static void hidden_state_changed_cb(GObject *object, ExpidusDesktop *desktop);
#endif
...
#ifdef ENABLE_DESKTOP_ICONS
static gdouble
expidus_desktop_ensure_system_font_size(ExpidusDesktop *desktop)
{
    GdkScreen *gscreen;
...
static void
expidus_desktop_setup_icon_view(ExpidusDesktop *desktop)
{
    EsdesktopIconViewManager *manager = NULL;

    switch(desktop->priv->icons_style) {
        case EXPIDUS_DESKTOP_ICON_STYLE_NONE:
            /* nada */
            break;

        case EXPIDUS_DESKTOP_ICON_STYLE_WINDOWS:
            manager = esdesktop_window_icon_manager_new(desktop->priv->gscreen);
            break;

#ifdef ENABLE_FILE_ICONS
        case EXPIDUS_DESKTOP_ICON_STYLE_FILES:
            {
                GFile *file;
                ...
                manager = esdesktop_file_icon_manager_new(file, desktop->priv->channel);
                g_object_unref(file);
            }
            break;
#endif
        ...
        default:
            g_critical("Unusable ExpidusDesktopIconStyle: %d.  Unable to " \
    ...
    if(manager) {
        expidus_desktop_ensure_system_font_size(desktop);

        desktop->priv->icon_view = esdesktop_icon_view_new(manager);
        /* If the user set a custom font size, use it. Otherwise use the system
         * font size */
        esdesktop_icon_view_set_font_size(ESDESKTOP_ICON_VIEW(desktop->priv->icon_view),
                                          (!desktop->priv->icons_font_size_set)
                                          ? desktop->priv->system_font_size
                                          : desktop->priv->icons_font_size);
        if(desktop->priv->icons_size > 0) {
            esdesktop_icon_view_set_icon_size(ESDESKTOP_ICON_VIEW(desktop->priv->icon_view),
                                              desktop->priv->icons_size);
        }
        esdesktop_icon_view_set_center_text (ESDESKTOP_ICON_VIEW(desktop->priv->icon_view),
        ...
        esdesktop_icon_view_set_primary(ESDESKTOP_ICON_VIEW(desktop->priv->icon_view),
                                        desktop->priv->primary);

        if(desktop->priv->icons_style == EXPIDUS_DESKTOP_ICON_STYLE_FILES)
...
static void
set_imgfile_root_property(ExpidusDesktop *desktop, const gchar *filename,
                          gint monitor)
{
    GdkDisplay *display;
    ...
    g_snprintf(property_name, 128, ESDESKTOP_IMAGE_FILE_FMT, monitor);
    if(filename) {
...
{
    ExpidusDesktop *desktop = user_data;
    cairo_pattern_t *pattern;
    gint w, h;

    TRACE("entering");

    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(desktop), NULL);
    ...
    esdesktop_get_screen_dimensions (gscreen, &w, &h);
...
{
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(user_data);
    ...
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));
    ...
    current_workspace = expidus_desktop_get_current_workspace(desktop);

    /* Find out which monitor the backdrop is on */
    for(i = 0; i < expidus_desktop_get_n_monitors(desktop); i++) {
        if(backdrop == expidus_workspace_get_backdrop(desktop->priv->workspaces[current_workspace], i)) {
            monitor = i;
            break;
        }
    }
    ...
    if(monitor == 0)
    {
        if (expidus_desktop_get_current_workspace(desktop) == 0)
        {
            new_filename = g_strdup(expidus_backdrop_get_image_filename(backdrop));
            if (g_strcmp0(desktop->priv->last_filename, new_filename) != 0)
            {
                desktop->priv->last_filename = g_strdup(new_filename);
                set_accountsservice_user_bg(expidus_backdrop_get_image_filename(backdrop));
            }
            g_free(new_filename);
        }
    }
    ...
    if(expidus_desktop_get_n_monitors(desktop) > 1
       && expidus_workspace_get_xinerama_stretch(desktop->priv->workspaces[current_workspace])) {
        /* Spanning screens */
         ...
         */
        for(i = 1; i < expidus_desktop_get_n_monitors(desktop); i++) {
        ...
        esdesktop_get_screen_dimensions (gscreen, &rect.width, &rect.height);
    ...
    expidus_backdrop_set_size(backdrop, rect.width, rect.height);
    ...
    if(rect.width != 0 && rect.height != 0) {
        ...
        GdkPixbuf *pix = expidus_backdrop_get_pixbuf(backdrop);
        cairo_t *cr;

        /* create the backdrop if needed */
        if(!pix) {
            expidus_backdrop_generate_async(backdrop);
...
screen_size_changed_cb(GdkScreen *gscreen, gpointer user_data)
{
    ...
    if(expidus_workspace_get_xinerama_stretch(desktop->priv->workspaces[current_workspace])) {
       backdrop_changed_cb(expidus_workspace_get_backdrop(desktop->priv->workspaces[current_workspace], 0), desktop);
    } else {
        gint i;

        for(i = 0; i < expidus_desktop_get_n_monitors(desktop); i++) {
            ExpidusBackdrop *current_backdrop;
            current_backdrop = expidus_workspace_get_backdrop(desktop->priv->workspaces[current_workspace], i);
...
static void
expidus_desktop_monitors_changed(GdkScreen *gscreen,
                              gpointer user_data)
{
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(user_data);
    gint i;
    ...
    for(i = 0; i < desktop->priv->nworkspaces; i++) {
        expidus_workspace_monitors_changed(desktop->priv->workspaces[i],
                                        gscreen);
    }
...
static void
workspace_backdrop_changed_cb(ExpidusWorkspace *workspace,
                              ...
                              gpointer user_data)
{
    ...
    g_return_if_fail(EXPIDUS_IS_WORKSPACE(workspace) && EXPIDUS_IS_BACKDROP(backdrop));
    ...
    if(expidus_desktop_get_current_workspace(desktop) == expidus_workspace_get_workspace_num(workspace)) {
        /* Update the backdrop!
         * In spanning mode, ignore updates to monitors other than the primary
         */
        if(!expidus_workspace_get_xinerama_stretch(workspace) || monitor == 0) {
                     ...
                     gpointer user_data)
{
    ...
    new_workspace = expidus_desktop_get_current_workspace(desktop);
    ...
    for(i = 0; i < expidus_desktop_get_n_monitors(desktop); i++) {
        backdrop = expidus_workspace_get_backdrop(desktop->priv->workspaces[new_workspace], i);
        /* update it */
        backdrop_changed_cb(backdrop, user_data);

        /* When we're spanning screens we only care about the first monitor */
        if(expidus_workspace_get_xinerama_stretch(desktop->priv->workspaces[new_workspace]))
            break;
    }
}
                     ...
                     gpointer user_data)
{
    ...
    desktop->priv->workspaces = g_realloc(desktop->priv->workspaces,
                                          desktop->priv->nworkspaces * sizeof(ExpidusWorkspace *));

    /* create the new workspace and set it up */
    desktop->priv->workspaces[nlast_workspace] = expidus_workspace_new(desktop->priv->gscreen,
                                                                    desktop->priv->channel,
                                                                    desktop->priv->property_prefix,
                                                                    nlast_workspace);

    expidus_workspace_monitors_changed(desktop->priv->workspaces[nlast_workspace],
                     ...
                     gpointer user_data)
{
    ...
    g_return_if_fail(EXPIDUS_IS_WORKSPACE(desktop->priv->workspaces[desktop->priv->nworkspaces-1]));
...
static void
screen_set_selection(ExpidusDesktop *desktop)
{
    Window xwin;
    gint xscreen;
    ...
    g_snprintf(selection_name, 100, ESDESKTOP_SELECTION_FMT, xscreen);
...
G_DEFINE_TYPE_WITH_PRIVATE(ExpidusDesktop, expidus_desktop, GTK_TYPE_WINDOW)


static void
expidus_desktop_class_init(ExpidusDesktopClass *klass)
{
    GObjectClass *gobject_class = (GObjectClass *)klass;
    GtkWidgetClass *widget_class = (GtkWidgetClass *)klass;

    gobject_class->finalize = expidus_desktop_finalize;
    gobject_class->set_property = expidus_desktop_set_property;
    gobject_class->get_property = expidus_desktop_get_property;

    widget_class->realize = expidus_desktop_realize;
    widget_class->unrealize = expidus_desktop_unrealize;
    widget_class->button_press_event = expidus_desktop_button_press_event;
    widget_class->button_release_event = expidus_desktop_button_release_event;
    widget_class->draw = expidus_desktop_draw;
    widget_class->delete_event = expidus_desktop_delete_event;
    widget_class->popup_menu = expidus_desktop_popup_menu;
    widget_class->style_updated = expidus_desktop_style_updated;

    signals[SIG_POPULATE_ROOT_MENU] = g_signal_new("populate-root-menu",
                                                   EXPIDUS_TYPE_DESKTOP,
                                                   G_SIGNAL_RUN_LAST,
                                                   G_STRUCT_OFFSET(ExpidusDesktopClass,
                                                                   populate_root_menu),
                                                   NULL, NULL,
...
#define ESDESKTOP_PARAM_FLAGS  (G_PARAM_READWRITE \
    ...
    g_object_class_install_property(gobject_class, PROP_ICON_STYLE,
                                    g_param_spec_enum("icon-style",
                                                      "icon style",
                                                      "icon style",
                                                      EXPIDUS_TYPE_DESKTOP_ICON_STYLE,
#ifdef ENABLE_FILE_ICONS
...
static void
expidus_desktop_init(ExpidusDesktop *desktop)
{
    desktop->priv = expidus_desktop_get_instance_private(desktop);
...
static void
expidus_desktop_finalize(GObject *object)
{
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(object);
    ...
    G_OBJECT_CLASS(expidus_desktop_parent_class)->finalize(object);
}

static void
expidus_desktop_set_property(GObject *object,
                          guint property_id,
                          const GValue *value,
                          GParamSpec *pspec)
{
    ...
    switch(property_id) {
        ...
        case PROP_ICON_STYLE:
            expidus_desktop_set_icon_style(desktop,
                                        g_value_get_enum(value));
            break;

        case PROP_ICON_SIZE:
            expidus_desktop_set_icon_size(desktop,
                                       g_value_get_uint(value));
            break;

        case PROP_ICON_ON_PRIMARY:
            expidus_desktop_set_primary(desktop,
                                       g_value_get_boolean(value));
            break;

        case PROP_ICON_FONT_SIZE:
            expidus_desktop_set_icon_font_size(desktop,
                                            g_value_get_uint(value));
            break;

        case PROP_ICON_FONT_SIZE_SET:
            expidus_desktop_set_use_icon_font_size(desktop,
                                                g_value_get_boolean(value));
            break;

        case PROP_ICON_CENTER_TEXT:
            expidus_desktop_set_center_text(desktop,
                                         g_value_get_boolean(value));
            break;

#endif
        case PROP_SINGLE_WORKSPACE_MODE:
            expidus_desktop_set_single_workspace_mode(desktop,
                                                   g_value_get_boolean(value));
            break;

        case PROP_SINGLE_WORKSPACE_NUMBER:
            expidus_desktop_set_single_workspace_number(desktop,
...
static void
expidus_desktop_get_property(GObject *object,
                          guint property_id,
                          GValue *value,
                          GParamSpec *pspec)
{
...
static void
expidus_desktop_realize(GtkWidget *widget)
{
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(widget);
    GdkAtom atom;
    gint sw, sh;
    Window xid;
    GdkWindow *groot;
    ...
    esdesktop_get_screen_dimensions (desktop->priv->gscreen, &sw, &sh);

    g_signal_connect(G_OBJECT(desktop->priv->gscreen),
                     "monitors-changed",
                     G_CALLBACK(expidus_desktop_monitors_changed),
                     desktop);

    /* chain up */
    GTK_WIDGET_CLASS(expidus_desktop_parent_class)->realize(widget);
    ...
    gdk_property_change(groot,
            gdk_atom_intern("EXPIDUS_DESKTOP_WINDOW", FALSE),
    ...
    esconf_g_property_bind(desktop->priv->channel,
...
#ifdef ENABLE_DESKTOP_ICONS
    expidus_desktop_setup_icon_view(desktop);
#endif
...
static void
expidus_desktop_unrealize(GtkWidget *widget)
{
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(widget);
    GdkDisplay  *display;
    gint i;
    ...
    /* disconnect all the esconf settings to this desktop */
    esconf_g_property_unbind_all(G_OBJECT(desktop));
    ...
    groot = gdk_screen_get_root_window(desktop->priv->gscreen);
    gdk_property_delete(groot, gdk_atom_intern("EXPIDUS_DESKTOP_WINDOW", FALSE));
...
G_GNUC_END_IGNORE_DEPRECATIONS
    ...
    if(desktop->priv->workspaces) {
        for(i = 0; i < desktop->priv->nworkspaces; i++) {
            g_snprintf(property_name, 128, ESDESKTOP_IMAGE_FILE_FMT, i);
...
static gboolean
expidus_desktop_button_press_event(GtkWidget *w,
                                GdkEventButton *evt)
{
    guint button = evt->button;
    guint state = evt->state;
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(w);

    DBG("entering");

    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(w), FALSE);

    if(evt->type == GDK_BUTTON_PRESS) {
        if(button == 3 || (button == 1 && (state & GDK_SHIFT_MASK))) {
#ifdef ENABLE_DESKTOP_ICONS
            /* Let the icon view handle these menu pop ups */
            if(desktop->priv->icons_style != EXPIDUS_DESKTOP_ICON_STYLE_NONE)
                return FALSE;
#endif
            ...
            expidus_desktop_popup_root_menu(desktop, button, evt->time);
            return TRUE;
                                  ...
                                  && (state & GDK_CONTROL_MASK)))
        {
            ...
            expidus_desktop_popup_secondary_root_menu(desktop, button, evt->time);
            return TRUE;
        }
    }
...
static gboolean
expidus_desktop_button_release_event(GtkWidget *w,
                                  GdkEventButton *evt)
{
    DBG("entering");
...
static gboolean
expidus_desktop_popup_menu(GtkWidget *w)
{
    GdkEventButton *evt;
    guint button, etime;
    ...
    expidus_desktop_popup_root_menu(EXPIDUS_DESKTOP(w), button, etime);
...
static gboolean
expidus_desktop_draw(GtkWidget *w,
                  cairo_t *cr)
{
    GList *children, *l;
...
static gboolean
expidus_desktop_delete_event(GtkWidget *w,
                          GdkEventAny *evt)
{
    if(EXPIDUS_DESKTOP(w)->priv->session_logout_func)
        EXPIDUS_DESKTOP(w)->priv->session_logout_func();
...
style_refresh_cb(gpointer user_data)
{
    ...
    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(desktop), FALSE);
...
G_GNUC_END_IGNORE_DEPRECATIONS
    ...
    if(!desktop->priv->icon_view || !ESDESKTOP_IS_ICON_VIEW(desktop->priv->icon_view))
        return FALSE;
    ...
    if(expidus_desktop_ensure_system_font_size(desktop) != old_font_size
...
static void
expidus_desktop_style_updated(GtkWidget *w)
{
#ifdef ENABLE_DESKTOP_ICONS
    ExpidusDesktop *desktop = EXPIDUS_DESKTOP(w);
...
static void
expidus_desktop_connect_settings(ExpidusDesktop *desktop)
{
#ifdef ENABLE_DESKTOP_ICONS
#define ICONS_PREFIX "/desktop-icons/"
    EsconfChannel *channel = desktop->priv->channel;

    expidus_desktop_freeze_updates(desktop);

    esconf_g_property_bind(channel, ICONS_PREFIX "style",
                           EXPIDUS_TYPE_DESKTOP_ICON_STYLE,
                           G_OBJECT(desktop), "icon-style");
    esconf_g_property_bind(channel, ICONS_PREFIX "icon-size", G_TYPE_UINT,
                           G_OBJECT(desktop), "icon-size");
    esconf_g_property_bind(channel, ICONS_PREFIX "primary", G_TYPE_BOOLEAN,
                           G_OBJECT(desktop), "primary");
    esconf_g_property_bind(channel, ICONS_PREFIX "font-size", G_TYPE_UINT,
                           G_OBJECT(desktop), "icon-font-size");
    esconf_g_property_bind(channel, ICONS_PREFIX "use-custom-font-size",
                           G_TYPE_BOOLEAN,
                           G_OBJECT(desktop), "icon-font-size-set");
    esconf_g_property_bind(channel, ICONS_PREFIX "center-text",
                           G_TYPE_BOOLEAN,
                           G_OBJECT(desktop), "icon-center-text");

    expidus_desktop_thaw_updates(desktop);
#undef ICONS_PREFIX
#endif
}

static gboolean
expidus_desktop_get_single_workspace_mode(ExpidusDesktop *desktop)
{
    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(desktop), TRUE);

    return desktop->priv->single_workspace_mode;
}

static gint
expidus_desktop_get_current_workspace(ExpidusDesktop *desktop)
{
    WnckWorkspace *wnck_workspace;
    gint workspace_num, current_workspace;

    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(desktop), -1);
    ...
    if(expidus_desktop_get_single_workspace_mode(desktop) &&
 ...
 *
 * Creates a new #ExpidusDesktop for the specified #GdkScreen.  If @gscreen is
 * %NULL, the default screen will be used.
 *
 * Return value: A new #ExpidusDesktop.
 **/
GtkWidget *
expidus_desktop_new(GdkScreen *gscreen,
                 EsconfChannel *channel,
                 const gchar *property_prefix)
{
    ExpidusDesktop *desktop;

    g_return_val_if_fail(channel && property_prefix, NULL);

    desktop = g_object_new(EXPIDUS_TYPE_DESKTOP, NULL);
    ...
    desktop->priv->channel = ESCONF_CHANNEL(g_object_ref(G_OBJECT(channel)));
    desktop->priv->property_prefix = g_strdup(property_prefix);

    expidus_desktop_connect_settings(desktop);
...
gint
expidus_desktop_get_n_monitors(ExpidusDesktop *desktop)
{
    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(desktop), 0);

    return gdk_display_get_n_monitors(gdk_screen_get_display(desktop->priv->gscreen));
}

void
expidus_desktop_set_icon_style(ExpidusDesktop *desktop,
                            ExpidusDesktopIconStyle style)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop)
                     && style <= EXPIDUS_DESKTOP_ICON_STYLE_FILES);
...
hidden_idle_cb(gpointer user_data)
{
    ...
    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(user_data), FALSE);

    desktop = EXPIDUS_DESKTOP(user_data);
...
expidus_desktop_get_icon_style(ExpidusDesktop *desktop)
{
    g_return_val_if_fail(EXPIDUS_IS_DESKTOP(desktop), EXPIDUS_DESKTOP_ICON_STYLE_NONE);

#ifdef ENABLE_DESKTOP_ICONS
    return desktop->priv->icons_style;
#else
    return EXPIDUS_DESKTOP_ICON_STYLE_NONE;
#endif
}

void
expidus_desktop_set_icon_size(ExpidusDesktop *desktop,
                           guint icon_size)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));
...
void
expidus_desktop_set_primary(ExpidusDesktop *desktop,
                           gboolean primary)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));
...
void
expidus_desktop_set_icon_font_size(ExpidusDesktop *desktop,
...
void
expidus_desktop_set_use_icon_font_size(ExpidusDesktop *desktop,
...
void
expidus_desktop_set_center_text (ExpidusDesktop *desktop,
                              gboolean center_text)
{
    ...
    if(desktop->priv->icon_view) {
        esdesktop_icon_view_set_center_text (ESDESKTOP_ICON_VIEW(desktop->priv->icon_view), center_text);
    }
#endif
}

static void
expidus_desktop_set_single_workspace_mode(ExpidusDesktop *desktop,
                                       gboolean single_workspace)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));
...
static void
expidus_desktop_set_single_workspace_number(ExpidusDesktop *desktop,
                                         gint workspace_num)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));
...
void
expidus_desktop_set_session_logout_func(ExpidusDesktop *desktop,
...
void
expidus_desktop_freeze_updates(ExpidusDesktop *desktop)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));
...
void
expidus_desktop_thaw_updates(ExpidusDesktop *desktop)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));

    desktop->priv->updates_frozen = FALSE;
    if(gtk_widget_get_realized(GTK_WIDGET(desktop)))
        expidus_desktop_monitors_changed(desktop->priv->gscreen, desktop);
}

static gboolean
expidus_desktop_menu_destroy_idled(gpointer data)
{
...
static void
expidus_desktop_do_menu_popup(ExpidusDesktop *desktop,
                           guint button,
                           guint activate_time,
                           guint populate_signal)
{
    ...
    g_signal_connect_swapped(G_OBJECT(menu), "deactivate",
                             ...
                             (gpointer)expidus_desktop_menu_destroy_idled);
    ...
    expidus_gtk_menu_popup_until_mapped(GTK_MENU(menu), NULL, NULL, NULL, NULL, button, activate_time);
}


void
expidus_desktop_popup_root_menu(ExpidusDesktop *desktop,
                             guint button,
                             guint activate_time)
{
    DBG("entering");

    expidus_desktop_do_menu_popup(desktop, button, activate_time,
                               signals[SIG_POPULATE_ROOT_MENU]);

}

void
expidus_desktop_popup_secondary_root_menu(ExpidusDesktop *desktop,
                                       guint button,
                                       guint activate_time)
{
    DBG("entering");
...
void
expidus_desktop_refresh(ExpidusDesktop *desktop,
                     ...
                     gboolean all_monitors)
{
    ...
    if(!all_monitors) {
        ...
        current_monitor_num = esdesktop_get_current_monitor_num(display);
    }
    ...
    for(i = 0; i < expidus_desktop_get_n_monitors(desktop); i++) {
        ...
        backdrop = expidus_workspace_get_backdrop(desktop->priv->workspaces[current_workspace], i);

        if(advance_wallpaper) {
            /* We need to trigger a new wallpaper event */
            expidus_backdrop_force_cycle(backdrop);
        } else {
...
void
expidus_desktop_arrange_icons(ExpidusDesktop *desktop)
{
    g_return_if_fail(EXPIDUS_IS_DESKTOP(desktop));

#ifdef ENABLE_DESKTOP_ICONS
    g_return_if_fail(ESDESKTOP_IS_ICON_VIEW(desktop->priv->icon_view));

    esdesktop_icon_view_sort_icons(ESDESKTOP_ICON_VIEW(desktop->priv->icon_view));
#endif
}

gboolean
expidus_desktop_get_cycle_backdrop(ExpidusDesktop *desktop)
{
    gint           monitor_num;
    GdkDisplay    *display;
    ...
    display = gdk_screen_get_display(desktop->priv->gscreen);
    monitor_num = esdesktop_get_current_monitor_num(display);

    workspace = desktop->priv->workspaces[desktop->priv->current_workspace];
    backdrop = expidus_workspace_get_backdrop(workspace, monitor_num);

    return expidus_backdrop_get_cycle_backdrop(backdrop);
}
