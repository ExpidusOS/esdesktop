 ...
 * Additional portions taken from https://bugzilla.expidus.org/attachment.cgi?id=3751
 * which is in expidus1-panel git commit id 2a8de2b1b019eaef543e34764c999a409fe2bef9
 * and adapted for esdesktop.
 */
...
static void esdesktop_application_finalize(GObject *object);
...
static void cb_esdesktop_application_reload(GAction  *action,
                                            GVariant *parameter,
                                            gpointer  data);

static void cb_esdesktop_application_next(GAction  *action,
                                          GVariant *parameter,
                                          gpointer  data);

static void esdesktop_handle_quit_signals(gint sig, gpointer user_data);
static void cb_esdesktop_application_quit(GAction  *action,
                                          GVariant *parameter,
                                          gpointer  data);

static void cb_esdesktop_application_menu(GAction  *action,
                                          GVariant *parameter,
                                          gpointer  data);

static void cb_esdesktop_application_arrange(GAction  *action,
                                             GVariant *parameter,
                                             gpointer  data);

static void cb_esdesktop_application_debug(GAction  *action,
                                           GVariant *parameter,
                                           gpointer  data);
...
static void esdesktop_application_startup(GApplication *g_application);
static void esdesktop_application_start(EsdesktopApplication *app);
static void esdesktop_application_shutdown(GApplication *g_application);

static gboolean esdesktop_application_local_command_line(GApplication *g_application,
                                                         gchar ***arguments,
                                                         int *exit_status);
static gint esdesktop_application_command_line(GApplication *g_application,
...
struct _EsdesktopApplicationClass
{
    GApplicationClass parent;
};

const gchar *fallback_CSS =
"EsdesktopIconView.view {"
"	background: transparent;"
"	color: @theme_selected_fg_color;"
"	border-radius: 3px;"
"}"
"EsdesktopIconView.view:active {"
"	background: alpha(@theme_selected_bg_color, 0.5);"
"	text-shadow: 0 1px 1px black;"
"}"
"EsdesktopIconView.view .label {"
"	text-shadow: 1px 1px 2px black;"
"}"
"EsdesktopIconView.view .label:active {"
"	color: @theme_selected_fg_color;"
"}"
"EsdesktopIconView .rubberband {"
"	background: alpha(@theme_selected_bg_color, 0.2);"
"	border: 1px solid @theme_selected_bg_color;"
"	border-radius: 0;"
"}"
"EsdesktopIconView:active {"
"	color: @theme_selected_bg_color;"
"}";

G_DEFINE_TYPE(EsdesktopApplication, esdesktop_application, G_TYPE_APPLICATION)


static void
esdesktop_application_class_init(EsdesktopApplicationClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
    GApplicationClass *gapplication_class = G_APPLICATION_CLASS(klass);

    gobject_class->finalize = esdesktop_application_finalize;

    gapplication_class->startup = esdesktop_application_startup;
    gapplication_class->shutdown = esdesktop_application_shutdown;
    gapplication_class->local_command_line = esdesktop_application_local_command_line;
    gapplication_class->command_line = esdesktop_application_command_line;
}

static void
esdesktop_application_add_action(EsdesktopApplication *app, GAction *action)
{
    g_action_map_add_action(G_ACTION_MAP(app), action);
}

static void
esdesktop_application_init(EsdesktopApplication *app)
{
    ...
    g_signal_connect(action, "activate", G_CALLBACK(cb_esdesktop_application_reload), app);
    esdesktop_application_add_action(app, G_ACTION(action));
    g_object_unref(action);

    /* next action */
    action = g_simple_action_new("next", NULL);
    g_signal_connect(action, "activate", G_CALLBACK(cb_esdesktop_application_next), app);
    esdesktop_application_add_action(app, G_ACTION(action));
    g_object_unref(action);

    /* quit action */
    action = g_simple_action_new("quit", NULL);
    g_signal_connect(action, "activate", G_CALLBACK(cb_esdesktop_application_quit), app);
    esdesktop_application_add_action(app, G_ACTION(action));
    g_object_unref(action);

    /* menu action, parameter pops up primary (TRUE) or windowlist menu */
    action = g_simple_action_new("menu", G_VARIANT_TYPE_BOOLEAN);
    g_signal_connect(action, "activate", G_CALLBACK(cb_esdesktop_application_menu), app);
    esdesktop_application_add_action(app, G_ACTION(action));
    g_object_unref(action);

    /* arrange action */
    action = g_simple_action_new("arrange", NULL);
    g_signal_connect(action, "activate", G_CALLBACK(cb_esdesktop_application_arrange), app);
    esdesktop_application_add_action(app, G_ACTION(action));
    g_object_unref(action);

    /* debug action, parameter toggles debug state */
    action = g_simple_action_new("debug", G_VARIANT_TYPE_BOOLEAN);
    g_signal_connect(action, "activate", G_CALLBACK(cb_esdesktop_application_debug), app);
    esdesktop_application_add_action(app, G_ACTION(action));
    g_object_unref(action);
}

static void
esdesktop_application_finalize(GObject *object)
{
    G_OBJECT_CLASS(esdesktop_application_parent_class)->finalize(object);
}

/**
 * esdesktop_application_get:
 *
 * Singleton. Additional calls increase the reference count.
 *
 * Return value: #EsdesktopApplication, free with g_object_unref.
 **/
EsdesktopApplication *
esdesktop_application_get(void)
{
    static EsdesktopApplication *app = NULL;

    if(app == NULL) {
       app = g_object_new(ESDESKTOP_TYPE_APPLICATION,
                          "application-id", "com.expidus.esdesktop",
...
session_die(gpointer user_data)
{
    ...
    /* Ensure we always have a valid reference so we can quit esdesktop */
...
{
    EsdesktopApplication *app = ESDESKTOP_APPLICATION(data);

    TRACE("entering");

    /* If esdesktop never started there's nothing to reload, esdesktop will
     * now startup */
    if(!app->desktop)
        return FALSE;
    ...
    if(app->desktop)
        expidus_desktop_refresh(EXPIDUS_DESKTOP(app->desktop), FALSE, TRUE);
...
static void
cb_esdesktop_application_reload(GAction  *action,
                                GVariant *parameter,
                                gpointer  data)
{
...
static void
cb_esdesktop_application_next(GAction  *action,
                              GVariant *parameter,
                              gpointer  data)
{
    EsdesktopApplication *app;
    ...
    g_return_if_fail(ESDESKTOP_IS_APPLICATION(data));

    app = ESDESKTOP_APPLICATION(data);

    /* If esdesktop never started there's nothing to do here */
    if(!app->desktop)
        return;

    /* reload the desktop forcing the wallpaper to advance */
    if(app->desktop)
        expidus_desktop_refresh(EXPIDUS_DESKTOP(app->desktop), TRUE, TRUE);
}

static void
esdesktop_handle_quit_signals(gint sig,
                              gpointer user_data)
{
    TRACE("entering");

    g_return_if_fail(ESDESKTOP_IS_APPLICATION(user_data));

    session_die(user_data);
}

static void
cb_esdesktop_application_quit(GAction  *action,
                              GVariant *parameter,
                              gpointer  data)
{
    EsdesktopApplication *app;
    ...
    /* If the user told esdesktop to quit, set the restart style to something
     * where it won't restart itself */
    if(app->sm_client && EXPIDUS_IS_SM_CLIENT(app->sm_client)) {
        expidus_sm_client_set_restart_style(app->sm_client,
 ...
 * windowlist menu */
static void
cb_esdesktop_application_menu(GAction  *action,
                              GVariant *parameter,
                              gpointer  data)
{
    EsdesktopApplication *app;
    ...
    /* sanity checks */
    if(!data || !ESDESKTOP_IS_APPLICATION(data))
        return;
    ...
    if(popup_root_menu) {
        expidus_desktop_popup_root_menu(EXPIDUS_DESKTOP(app->desktop),
                                     0, GDK_CURRENT_TIME);
    } else {
        expidus_desktop_popup_secondary_root_menu(EXPIDUS_DESKTOP(app->desktop),
                                               0, GDK_CURRENT_TIME);
    }
}

static void
cb_esdesktop_application_arrange(GAction  *action,
                                 GVariant *parameter,
                                 gpointer  data)
{
    EsdesktopApplication *app;
    ...
    expidus_desktop_arrange_icons(EXPIDUS_DESKTOP(app->desktop));
}

/* parameter is a boolean that determines whether to enable or disable the
 * debug messages */
static void
cb_esdesktop_application_debug(GAction  *action,
                               GVariant *parameter,
                               gpointer  data)
{
    TRACE("entering");
    ...
    esdesktop_debug_set(g_variant_get_boolean(parameter));
}
...
cb_wait_for_window_manager(gpointer data)
{
    ...
    /* Check if it was canceled. This way esdesktop doesn't start up if
     * we're quitting */
...
cb_wait_for_window_manager_destroyed(gpointer data)
{
    ...
    if(!wfwm->have_wm) {
        g_printerr("No window manager registered on screen 0. "
                   "To start the esdesktop without this check, run with --disable-wm-check.\n");
    } else {
    ...
    esdesktop_application_start(wfwm->app);

    wait_for_window_manager_cleanup(wfwm);
}

static void
esdesktop_application_startup(GApplication *g_application)
{
    EsdesktopApplication *app = ESDESKTOP_APPLICATION(g_application);
    WaitForWM *wfwm;
    guint i;
    gchar **atom_names;
    ...
    if(!app->opt_disable_wm_check) {
        ...
        /* directly launch */
        esdesktop_application_start(app);
    }

    /* let the parent class do it's startup as well */
    G_APPLICATION_CLASS(esdesktop_application_parent_class)->startup(g_application);
}

static void
esdesktop_application_theme_changed (GtkSettings *settings,
                                     EsdesktopApplication *app)
{
    ...
    if (g_strrstr (css, "EsdesktopIconView") != NULL) {
        DBG("EsdesktopIconView section found in theme %s", theme);
        ...
        DBG("EsdesktopIconView section not found in theme %s, setting our fallback", theme);
        if (custom_provider != NULL) {
            gtk_style_context_remove_provider_for_screen (gdk_screen_get_default (),
                                                          GTK_STYLE_PROVIDER(custom_provider));
            g_clear_object (&custom_provider);
...
static void
esdesktop_application_start(EsdesktopApplication *app)
{
    ...
    g_signal_connect (settings, "notify::gtk-theme-name", G_CALLBACK (esdesktop_application_theme_changed), NULL);
    esdesktop_application_theme_changed (settings, app);
    ...
    app->sm_client = expidus_sm_client_get();
    expidus_sm_client_set_restart_style(app->sm_client, EXPIDUS_SM_CLIENT_RESTART_IMMEDIATELY);
    expidus_sm_client_set_priority(app->sm_client, EXPIDUS_SM_CLIENT_PRIORITY_DESKTOP);
    g_signal_connect(app->sm_client, "quit", G_CALLBACK(session_die), app);

    if(!expidus_sm_client_connect(app->sm_client, &error) && error) {
    ...
    if(!esconf_init(&error)) {
        ...
        app->channel = esconf_channel_get(ESDESKTOP_CHANNEL);

    g_snprintf(buf, sizeof(buf), "/backdrop/screen%d/", 0);
    app->desktop = expidus_desktop_new(gdk_display_get_default_screen(gdpy),
                                    app->channel, buf);
    ...
    menu_attach(EXPIDUS_DESKTOP(app->desktop));
    windowlist_attach(EXPIDUS_DESKTOP(app->desktop));
    ...
    expidus_desktop_set_session_logout_func(EXPIDUS_DESKTOP(app->desktop),
                                         session_logout);
...
gint
esdesktop_application_run(EsdesktopApplication *app, int argc, char **argv)
{
    return g_application_run(G_APPLICATION(app), argc, argv);
}

static void
esdesktop_application_shutdown(GApplication *g_application)
{
    EsdesktopApplication *app = ESDESKTOP_APPLICATION(g_application);
...
#ifdef HAVE_LIBNOTIFY
    esdesktop_notify_uninit();
#endif

    G_APPLICATION_CLASS(esdesktop_application_parent_class)->shutdown(g_application);
}

static gboolean
esdesktop_application_local_command_line(GApplication *g_application,
                                         gchar ***arguments,
                                         int *exit_status)
{
    EsdesktopApplication *app = ESDESKTOP_APPLICATION(g_application);
    ...
    const GOptionEntry main_entries[] = {
        ...
        { "quit", 'Q', G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_NONE, &opt_quit, N_("Cause esdesktop to quit"), NULL },
    ...
    g_option_context_add_group(octx, expidus_sm_client_get_option_group(argc, *arguments));
    ...
    /* This will call esdesktop_application_startup if it needs to */
    ...
    /* propagate it up */
    return G_APPLICATION_CLASS(esdesktop_application_parent_class)->local_command_line(g_application, arguments, exit_status);
}

static gint
esdesktop_application_command_line(GApplication *g_application,
                                   GApplicationCommandLine *command_line)
{
    /* If we don't process everything in the local command line then the options
     * won't show up during esdesktop --help */
