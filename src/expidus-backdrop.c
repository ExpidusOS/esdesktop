...
#include <libexpidus1util/libexpidus1util.h> /* for DBG/TRACE */

#include "expidus-backdrop.h"
#include "expidus-desktop-enum-types.h"
#include "esdesktop-common.h"  /* for DEFAULT_BACKDROP */

#ifndef abs
#define abs(x)  ( (x) < 0 ? -(x) : (x) )
#endif

#define EXPIDUS_BACKDROP_BUFFER_SIZE 32768

#ifndef O_BINARY
#define O_BINARY  0
#endif

typedef struct _ExpidusBackdropImageData ExpidusBackdropImageData;

static void expidus_backdrop_finalize(GObject *object);
static void expidus_backdrop_set_property(GObject *object,
                                       guint property_id,
                                       const GValue *value,
                                       GParamSpec *pspec);
static void expidus_backdrop_get_property(GObject *object,
                                       guint property_id,
                                       GValue *value,
                                       GParamSpec *pspec);
static gboolean expidus_backdrop_timer(gpointer user_data);

static GdkPixbuf *expidus_backdrop_generate_canvas(ExpidusBackdrop *backdrop);

static void expidus_backdrop_loader_size_prepared_cb(GdkPixbufLoader *loader,
                                                  gint width,
                                                  gint height,
                                                  gpointer user_data);

static void expidus_backdrop_loader_closed_cb(GdkPixbufLoader *loader,
                                           ExpidusBackdropImageData *image_data);

static void expidus_backdrop_file_ready_cb(GObject *source_object,
                                        GAsyncResult *res,
                                        gpointer user_data);

static void expidus_backdrop_file_input_stream_ready_cb(GObject *source_object,
                                                     GAsyncResult *res,
                                                     gpointer user_data);

static void expidus_backdrop_image_data_release(ExpidusBackdropImageData *image_data);

gchar *expidus_backdrop_choose_next         (ExpidusBackdrop *backdrop);
gchar *expidus_backdrop_choose_random       (ExpidusBackdrop *backdrop);
gchar *expidus_backdrop_choose_chronological(ExpidusBackdrop *backdrop);

static void cb_expidus_backdrop_image_files_changed(GFileMonitor     *monitor,
                                                 GFile            *file,
                                                 GFile            *other_file,
                                                 GFileMonitorEvent event,
                                                 gpointer          user_data);

struct _ExpidusBackdropPrivate
{
    gint width, height;
    gint bpp;
    ...
    ExpidusBackdropColorStyle color_style;
    GdkRGBA color1;
    GdkRGBA color2;

    ExpidusBackdropImageStyle image_style;
    gchar *image_path;
    ...
    ExpidusBackdropCyclePeriod cycle_period;
    gboolean random_backdrop_order;
};

struct _ExpidusBackdropImageData
{
        ...
        ExpidusBackdropColorStyle style)
{
    ...
    g_return_val_if_fail(style == EXPIDUS_BACKDROP_COLOR_HORIZ_GRADIENT || style == EXPIDUS_BACKDROP_COLOR_VERT_GRADIENT, NULL);

    root = gdk_screen_get_root_window(gdk_screen_get_default ());
    surface = gdk_window_create_similar_surface(root, CAIRO_CONTENT_COLOR_ALPHA, width, height);
    cr = cairo_create(surface);

    if(style == EXPIDUS_BACKDROP_COLOR_VERT_GRADIENT) {
...
void
expidus_backdrop_clear_cached_image(ExpidusBackdrop *backdrop)
{
    TRACE("entering");
...
static void
esdesktop_backdrop_clear_directory_monitor(ExpidusBackdrop *backdrop)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));

    if(backdrop->priv->monitor) {
            g_signal_handlers_disconnect_by_func(G_OBJECT(backdrop->priv->monitor),
                                                 G_CALLBACK(cb_expidus_backdrop_image_files_changed),
                                                 backdrop);
 ...
 * esdesktop-settings displays the images */
static gint
...
static void
cb_expidus_backdrop_image_files_changed(GFileMonitor     *monitor,
                                     GFile            *file,
                                     GFile            *other_file,
                                     GFileMonitorEvent event,
                                     gpointer          user_data)
{
    ExpidusBackdrop *backdrop = EXPIDUS_BACKDROP(user_data);
    gchar *changed_file = NULL;
    GList *item;

    switch(event) {
        case G_FILE_MONITOR_EVENT_CREATED:
            if(!expidus_backdrop_get_cycle_backdrop(backdrop)) {
             ...
             * anything */
            if(!esdesktop_image_file_is_valid(changed_file)) {
                g_free(changed_file);
                return;
            }

            if(!expidus_backdrop_get_random_order(backdrop)) {
        ...
        case G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT:
            ...
            if(g_strcmp0(changed_file, backdrop->priv->image_path) == 0) {
                ...
                expidus_backdrop_clear_cached_image(backdrop);
...
static GList *
list_image_files_in_dir(ExpidusBackdrop *backdrop, const gchar *filename)
{
    GDir *dir;
    ...
    while((file = g_dir_read_name(dir))) {
        gchar *current_file = g_strdup_printf(needs_slash ? "%s/%s" : "%s%s",
                                              dir_name, file);
        if(esdesktop_image_file_is_valid(current_file)) {
    ...
    if(file_count > 1 && !expidus_backdrop_get_random_order(backdrop)) {
...
static void
expidus_backdrop_load_image_files(ExpidusBackdrop *backdrop)
{
    TRACE("entering");
    ...
    if(backdrop->priv->image_files == NULL &&
       ...
       expidus_backdrop_get_cycle_backdrop(backdrop)) {
        backdrop->priv->image_files = list_image_files_in_dir(backdrop, backdrop->priv->image_path);

        esdesktop_backdrop_clear_directory_monitor(backdrop);
    }
...
gchar *
expidus_backdrop_choose_next(ExpidusBackdrop *backdrop)
{
    GList *current_file;
    ...
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), NULL);
 ...
 * returns NULL on fail. */
gchar *
expidus_backdrop_choose_random(ExpidusBackdrop *backdrop)
{
...
gchar *
expidus_backdrop_choose_chronological(ExpidusBackdrop *backdrop)
{
    GDateTime *datetime;
    GList *new_file;
...
G_DEFINE_TYPE_WITH_PRIVATE(ExpidusBackdrop, expidus_backdrop, G_TYPE_OBJECT)


static void
expidus_backdrop_class_init(ExpidusBackdropClass *klass)
{
    GObjectClass *gobject_class = (GObjectClass *)klass;

    gobject_class->finalize = expidus_backdrop_finalize;
    gobject_class->set_property = expidus_backdrop_set_property;
    gobject_class->get_property = expidus_backdrop_get_property;

    backdrop_signals[BACKDROP_CHANGED] = g_signal_new("changed",
            G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
            G_STRUCT_OFFSET(ExpidusBackdropClass, changed), NULL, NULL,
            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    backdrop_signals[BACKDROP_CYCLE] = g_signal_new("cycle",
            G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
            G_STRUCT_OFFSET(ExpidusBackdropClass, cycle), NULL, NULL,
            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    backdrop_signals[BACKDROP_READY] = g_signal_new("ready",
            G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
            G_STRUCT_OFFSET(ExpidusBackdropClass, ready), NULL, NULL,
            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
#define ESDESKTOP_PARAM_FLAGS  (G_PARAM_READWRITE \
     ...
     * expidus_workspace_migrate_backdrop_color_style can handle it properly */
    g_object_class_install_property(gobject_class, PROP_COLOR_STYLE,
                                    g_param_spec_enum("color-style",
                                                      "color style",
                                                      "color style",
                                                      EXPIDUS_TYPE_BACKDROP_COLOR_STYLE,
     ...
     * expidus_workspace_migrate_backdrop_image_style can handle it properly */
    g_object_class_install_property(gobject_class, PROP_IMAGE_STYLE,
                                    g_param_spec_enum("image-style",
                                                      "image style",
                                                      "image style",
                                                      EXPIDUS_TYPE_BACKDROP_IMAGE_STYLE,
                                                      EXPIDUS_BACKDROP_IMAGE_INVALID,
                                                      ESDESKTOP_PARAM_FLAGS));

    /* The DEFAULT_BACKDROP is provided in the function
     * expidus_workspace_migrate_backdrop_image instead of here */
    ...
    g_object_class_install_property(gobject_class, PROP_BACKDROP_CYCLE_PERIOD,
                                    g_param_spec_enum("backdrop-cycle-period",
                                                      ...
                                                      "backdrop-cycle-period",
                                                      EXPIDUS_TYPE_BACKDROP_CYCLE_PERIOD,
...
static void
expidus_backdrop_init(ExpidusBackdrop *backdrop)
{
    backdrop->priv = expidus_backdrop_get_instance_private(backdrop);
    backdrop->priv->cycle_timer_id = 0;
...
static void
expidus_backdrop_finalize(GObject *object)
{
    ExpidusBackdrop *backdrop = EXPIDUS_BACKDROP(object);
    ...
    G_OBJECT_CLASS(expidus_backdrop_parent_class)->finalize(object);
}

static void
expidus_backdrop_set_property(GObject *object,
                           guint property_id,
                           const GValue *value,
                           GParamSpec *pspec)
{
    ...
    switch(property_id) {
        case PROP_COLOR_STYLE:
            expidus_backdrop_set_color_style(backdrop, g_value_get_enum(value));
            break;

        case PROP_COLOR1:
            color = g_value_get_boxed(value);
            if(color)
                expidus_backdrop_set_first_color(backdrop, color);
            break;

        case PROP_COLOR2:
            color = g_value_get_boxed(value);
            if(color)
                expidus_backdrop_set_second_color(backdrop, color);
            break;

        case PROP_IMAGE_STYLE:
            expidus_backdrop_set_image_style(backdrop, g_value_get_enum(value));
            break;

        case PROP_IMAGE_FILENAME:
            expidus_backdrop_set_image_filename(backdrop,
                                             g_value_get_string(value));
            break;

        case PROP_BACKDROP_CYCLE_ENABLE:
            expidus_backdrop_set_cycle_backdrop(backdrop, g_value_get_boolean(value));
            break;

        case PROP_BACKDROP_CYCLE_PERIOD:
            expidus_backdrop_set_cycle_period(backdrop, g_value_get_enum(value));
            break;

        case PROP_BACKDROP_CYCLE_TIMER:
            expidus_backdrop_set_cycle_timer(backdrop, g_value_get_uint(value));
            break;

        case PROP_BACKDROP_RANDOM_ORDER:
            expidus_backdrop_set_random_order(backdrop, g_value_get_boolean(value));
            break;
...
static void
expidus_backdrop_get_property(GObject *object,
                           guint property_id,
                           GValue *value,
                           GParamSpec *pspec)
{
    ...
    switch(property_id) {
        case PROP_COLOR_STYLE:
            g_value_set_enum(value, expidus_backdrop_get_color_style(backdrop));
            break;
        ...
        case PROP_IMAGE_STYLE:
            g_value_set_enum(value, expidus_backdrop_get_image_style(backdrop));
            break;

        case PROP_IMAGE_FILENAME:
            g_value_set_string(value,
                               expidus_backdrop_get_image_filename(backdrop));
            break;

        case PROP_BACKDROP_CYCLE_ENABLE:
            g_value_set_boolean(value, expidus_backdrop_get_cycle_backdrop(backdrop));
            break;

        case PROP_BACKDROP_CYCLE_PERIOD:
            g_value_set_enum(value, expidus_backdrop_get_cycle_period(backdrop));
            break;

        case PROP_BACKDROP_CYCLE_TIMER:
            g_value_set_uint(value, expidus_backdrop_get_cycle_timer(backdrop));
            break;

        case PROP_BACKDROP_RANDOM_ORDER:
            g_value_set_boolean(value, expidus_backdrop_get_random_order(backdrop));
            break;
...
/**
 * expidus_backdrop_new:
 * @visual: The current X visual in use.
 *
 * Creates a new #ExpidusBackdrop.  The @visual parameter is needed to decide the
 * optimal dithering method.
 *
 * Return value: A new #ExpidusBackdrop.
 **/
ExpidusBackdrop *
expidus_backdrop_new(GdkVisual *visual)
{
    ExpidusBackdrop *backdrop;

    g_return_val_if_fail(GDK_IS_VISUAL(visual), NULL);

    backdrop = g_object_new(EXPIDUS_TYPE_BACKDROP, NULL);
...
/**
 * expidus_backdrop_new_with_size:
 * @width: The width of the #ExpidusBackdrop.
 * @height: The height of the #ExpidusBackdrop.
 *
 * Creates a new #ExpidusBackdrop with the specified @width and @height.
 *
 * Return value: A new #ExpidusBackdrop.
 **/
ExpidusBackdrop *
expidus_backdrop_new_with_size(GdkVisual *visual,
                            gint width,
                            gint height)
{
    ExpidusBackdrop *backdrop;
...
/**
 * expidus_backdrop_set_size:
 * @backdrop: An #ExpidusBackdrop.
 * @width: The new width.
 * @height: The new height.
 *
 * Sets the backdrop's size, e.g., after a screen size change.  Note: This will
 * not emit the 'changed' signal; owners of #ExpidusBackdrop objects are expected
 * to manually refresh the backdrop data after calling expidus_backdrop_set_size().
 **/
void
expidus_backdrop_set_size(ExpidusBackdrop *backdrop, gint width, gint height)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));
...
/**
 * expidus_backdrop_set_color_style:
 * @backdrop: An #ExpidusBackdrop.
 * @style: An #ExpidusBackdropColorStyle.
 *
 * Sets the color style for the #ExpidusBackdrop to the specified @style.
 **/
void
expidus_backdrop_set_color_style(ExpidusBackdrop *backdrop,
        ExpidusBackdropColorStyle style)
{
    TRACE("entering");

    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));
    g_return_if_fail((int)style >= -1 && style <= EXPIDUS_BACKDROP_COLOR_TRANSPARENT);
...
expidus_backdrop_get_color_style(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop),
                         EXPIDUS_BACKDROP_COLOR_SOLID);
    return backdrop->priv->color_style;
}

/**
 * expidus_backdrop_set_first_color:
 * @backdrop: An #ExpidusBackdrop.
 * @color: A #GdkRGBA.
 *
 * Sets the "first" color for the #ExpidusBackdrop.  This is the color used if
 * the color style is set to EXPIDUS_BACKDROP_COLOR_SOLID.  It is used as the
 * left-side color or top color if the color style is set to
 * EXPIDUS_BACKDROP_COLOR_HORIZ_GRADIENT or EXPIDUS_BACKDROP_COLOR_VERT_GRADIENT,
 * respectively.
 **/
void
expidus_backdrop_set_first_color(ExpidusBackdrop *backdrop,
                              const GdkRGBA *color)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop) && color != NULL);
...
void
expidus_backdrop_get_first_color(ExpidusBackdrop *backdrop,
                              GdkRGBA *color)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop) && color);

    memcpy(color, &backdrop->priv->color1, sizeof(GdkRGBA));
}

/**
 * expidus_backdrop_set_second_color:
 * @backdrop: An #ExpidusBackdrop.
 * @color: A #GdkRGBA.
 *
 * Sets the "second" color for the #ExpidusBackdrop.  This is the color used as the
 * right-side color or bottom color if the color style is set to
 * EXPIDUS_BACKDROP_COLOR_HORIZ_GRADIENT or EXPIDUS_BACKDROP_COLOR_VERT_GRADIENT,
 * respectively.
 **/
void
expidus_backdrop_set_second_color(ExpidusBackdrop *backdrop,
                               const GdkRGBA *color)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop) && color != NULL);
            ...
            || color->alpha != backdrop->priv->color2.alpha)
    {
        ...
        if(backdrop->priv->color_style != EXPIDUS_BACKDROP_COLOR_SOLID)
            g_signal_emit(G_OBJECT(backdrop), backdrop_signals[BACKDROP_CHANGED], 0);
    }
}

void
expidus_backdrop_get_second_color(ExpidusBackdrop *backdrop,
                               GdkRGBA *color)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop) && color);
...
/**
 * expidus_backdrop_set_image_style:
 * @backdrop: An #ExpidusBackdrop.
 * @style: An ExpidusBackdropImageStyle.
 *
 * Sets the image style to be used for the #ExpidusBackdrop.
 * "STRETCHED" will stretch the image to the full width and height of the
 * #ExpidusBackdrop, while "SCALED" will resize the image to fit the desktop
 * while maintaining the image's aspect ratio.
 **/
void
expidus_backdrop_set_image_style(ExpidusBackdrop *backdrop,
...
expidus_backdrop_get_image_style(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop),
...
/**
 * expidus_backdrop_set_image_filename:
 * @backdrop: An #ExpidusBackdrop.
 * @filename: A filename.
 *
 * Sets the image that should be used with the #ExpidusBackdrop.  The image will
 ...
 **/
void
expidus_backdrop_set_image_filename(ExpidusBackdrop *backdrop, const gchar *filename)
{
    ...
    expidus_backdrop_load_image_files(backdrop);

    g_signal_emit(G_OBJECT(backdrop), backdrop_signals[BACKDROP_CHANGED], 0);
}

const gchar *
expidus_backdrop_get_image_filename(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), NULL);
...
static void
expidus_backdrop_cycle_backdrop(ExpidusBackdrop *backdrop)
{
    ...
    if(period == EXPIDUS_BACKDROP_PERIOD_CHRONOLOGICAL) {
        /* chronological first */
        new_backdrop = expidus_backdrop_choose_chronological(backdrop);
    } else if(backdrop->priv->random_backdrop_order) {
        /* then random */
        new_backdrop = expidus_backdrop_choose_random(backdrop);
    } else {
        /* sequential, the default */
        new_backdrop = expidus_backdrop_choose_next(backdrop);
    }

    /* Only emit the cycle signal if something changed */
    if(g_strcmp0(backdrop->priv->image_path, new_backdrop) != 0) {
        expidus_backdrop_set_image_filename(backdrop, new_backdrop);
...
static void
expidus_backdrop_remove_backdrop_timer(ExpidusBackdrop *backdrop)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));
...
static gboolean
expidus_backdrop_timer(gpointer user_data)
{
    ExpidusBackdrop *backdrop = user_data;
    ...
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), FALSE);

    /* Don't bother with trying to cycle a backdrop if we're not using images */
    if(backdrop->priv->image_style != EXPIDUS_BACKDROP_IMAGE_NONE)
        expidus_backdrop_cycle_backdrop(backdrop);

    /* Now to complicate things; we have to handle some special cases */
    switch(backdrop->priv->cycle_period) {
        case EXPIDUS_BACKDROP_PERIOD_STARTUP:
            /* no more cycling */
            return FALSE;

        case EXPIDUS_BACKDROP_PERIOD_CHRONOLOGICAL:
        case EXPIDUS_BACKDROP_PERIOD_HOURLY:
        ...
        case EXPIDUS_BACKDROP_PERIOD_DAILY:
            local_time = g_date_time_new_now_local();
    ...
    if(cycle_interval != 0) {
        ...
        expidus_backdrop_remove_backdrop_timer(backdrop);
...
/**
 * expidus_backdrop_set_cycle_timer:
 * @backdrop: An #ExpidusBackdrop.
 ...
 **/
void
expidus_backdrop_set_cycle_timer(ExpidusBackdrop *backdrop, guint cycle_timer)
{
    GDateTime *local_time = NULL;
    guint cycle_interval = 0;
    gint hour, minute, second;
    ...
    if(backdrop->priv->cycle_timer != 0 && backdrop->priv->cycle_backdrop == TRUE) {
        switch(backdrop->priv->cycle_period) {
            case EXPIDUS_BACKDROP_PERIOD_SECONDS:
                cycle_interval = backdrop->priv->cycle_timer;
                break;

            case EXPIDUS_BACKDROP_PERIOD_MINUTES:
                cycle_interval = backdrop->priv->cycle_timer * 60;
                break;

            case EXPIDUS_BACKDROP_PERIOD_HOURS:
...
guint
expidus_backdrop_get_cycle_timer(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), 0);
    return backdrop->priv->cycle_timer;
}

/**
 * expidus_backdrop_set_cycle_backdrop:
 * @backdrop: An #ExpidusBackdrop.
 ...
 * the cycle_timer value set in expidus_backdrop_set_cycle_timer.
 **/
void
expidus_backdrop_set_cycle_backdrop(ExpidusBackdrop *backdrop,
                                 gboolean cycle_backdrop)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));
    ...
    if(backdrop->priv->cycle_backdrop != cycle_backdrop) {
        ...
        expidus_backdrop_set_cycle_timer(backdrop,
                                      expidus_backdrop_get_cycle_timer(backdrop));
...
gboolean
expidus_backdrop_get_cycle_backdrop(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), 0);
...
/**
 * expidus_backdrop_set_cycle_period:
 * @backdrop: An #ExpidusBackdrop.
 ...
 * actual wall clock values or just at esdesktop's start up.
 **/
void
expidus_backdrop_set_cycle_period(ExpidusBackdrop *backdrop,
                               ExpidusBackdropCyclePeriod period)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));
    g_return_if_fail(period != EXPIDUS_BACKDROP_PERIOD_INVALID);
...
expidus_backdrop_get_cycle_period(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), EXPIDUS_BACKDROP_PERIOD_INVALID);

    return backdrop->priv->cycle_period;
}

/**
 * expidus_backdrop_set_random_order:
 * @backdrop: An #ExpidusBackdrop.
 ...
 **/
void
expidus_backdrop_set_random_order(ExpidusBackdrop *backdrop,
...
gboolean
expidus_backdrop_get_random_order(ExpidusBackdrop *backdrop)
{
    g_return_val_if_fail(EXPIDUS_IS_BACKDROP(backdrop), FALSE);
...
void
expidus_backdrop_force_cycle(ExpidusBackdrop *backdrop)
{
    g_return_if_fail(EXPIDUS_IS_BACKDROP(backdrop));
...
expidus_backdrop_generate_canvas(ExpidusBackdrop *backdrop)
{
    ...
    if(backdrop->priv->color_style == EXPIDUS_BACKDROP_COLOR_INVALID) {
        g_warning("expidus_backdrop_generate_canvas: Invalid color style");
        backdrop->priv->color_style = EXPIDUS_BACKDROP_COLOR_SOLID;
    }

    if(backdrop->priv->color_style == EXPIDUS_BACKDROP_COLOR_SOLID)
        final_image = create_solid(&backdrop->priv->color1, w, h);
    else if(backdrop->priv->color_style == EXPIDUS_BACKDROP_COLOR_TRANSPARENT) {
...
static void
expidus_backdrop_image_data_release(ExpidusBackdropImageData *image_data)
{
    TRACE("entering");
    ...
    if(EXPIDUS_IS_BACKDROP(image_data->backdrop)) {
...
/**
 * expidus_backdrop_get_pixbuf:
 * @backdrop: An #ExpidusBackdrop.
 *
 * Returns the composited backdrop image if one has been generated. If it
 * returns NULL, call expidus_backdrop_generate_async to create the pixbuf.
 * Free with g_object_unref() when you are finished.
 **/
GdkPixbuf *
expidus_backdrop_get_pixbuf(ExpidusBackdrop *backdrop)
{
    TRACE("entering");
    ...
    /* !backdrop->priv->pix, call expidus_backdrop_generate_async */
    return NULL;
}

/**
 * expidus_backdrop_generate_async:
 * @backdrop: An #ExpidusBackdrop.
 *
 * Generates the final composited, resized image from the #ExpidusBackdrop.
 * Emits the "ready" signal when the image has been created.
 **/
void
expidus_backdrop_generate_async(ExpidusBackdrop *backdrop)
{
    GFile *file;
    ExpidusBackdropImageData *image_data = NULL;
    ...
    if(backdrop->priv->image_style == EXPIDUS_BACKDROP_IMAGE_NONE) {
        backdrop->priv->pix = expidus_backdrop_generate_canvas(backdrop);
    ...
    image_data = g_new0(ExpidusBackdropImageData, 1);
    ...
    image_data->image_buffer = g_new0(guchar, EXPIDUS_BACKDROP_BUFFER_SIZE);

    g_signal_connect(image_data->loader, "size-prepared", G_CALLBACK(expidus_backdrop_loader_size_prepared_cb), image_data);
    g_signal_connect(image_data->loader, "closed", G_CALLBACK(expidus_backdrop_loader_closed_cb), image_data);

    g_file_read_async(file,
                      G_PRIORITY_DEFAULT,
                      image_data->cancellable,
                      expidus_backdrop_file_ready_cb,
                      image_data);
}


static void
expidus_backdrop_loader_size_prepared_cb(GdkPixbufLoader *loader,
                                      gint width,
                                      gint height,
                                      gpointer user_data)
{
    ExpidusBackdropImageData *image_data = user_data;
    ExpidusBackdrop *backdrop;
    ...
    if(g_cancellable_is_cancelled(image_data->cancellable)) {
        expidus_backdrop_image_data_release(image_data);
        g_free(image_data);
        return;
    }

    /* invalid backdrop? quit but don't free image data */
    if(!EXPIDUS_IS_BACKDROP(backdrop)) {
        return;
    }

    if(backdrop->priv->image_style == EXPIDUS_BACKDROP_IMAGE_INVALID) {
        g_warning("Invalid image style, setting to EXPIDUS_BACKDROP_IMAGE_ZOOMED");
        backdrop->priv->image_style = EXPIDUS_BACKDROP_IMAGE_ZOOMED;
    }

    switch(backdrop->priv->image_style) {
        case EXPIDUS_BACKDROP_IMAGE_CENTERED:
        case EXPIDUS_BACKDROP_IMAGE_TILED:
        case EXPIDUS_BACKDROP_IMAGE_NONE:
            /* do nothing */
            break;

        case EXPIDUS_BACKDROP_IMAGE_STRETCHED:
            gdk_pixbuf_loader_set_size(loader,
                                       backdrop->priv->width,
                                       backdrop->priv->height);
            break;

        case EXPIDUS_BACKDROP_IMAGE_SCALED:
        ...
        case EXPIDUS_BACKDROP_IMAGE_ZOOMED:
        case EXPIDUS_BACKDROP_IMAGE_SPANNING_SCREENS:
            xscale = (gdouble)backdrop->priv->width / width;
            yscale = (gdouble)backdrop->priv->height / height;
            if(xscale < yscale) {
                xscale = yscale;
...
static void
expidus_backdrop_loader_closed_cb(GdkPixbufLoader *loader,
                               ExpidusBackdropImageData *image_data)
{
    ExpidusBackdrop *backdrop = image_data->backdrop;
    GdkPixbuf *final_image = NULL, *image = NULL, *tmp = NULL;
    gint i, j;
    gint w, h, iw = 0, ih = 0;
    ExpidusBackdropImageStyle istyle;
    gint dx, dy, xo, yo;
    gdouble xscale, yscale;
     ...
     * any scaling at all */
    if(w == iw && h == ih)
        istyle = EXPIDUS_BACKDROP_IMAGE_CENTERED;
     ...
     * some images.  https://bugzilla.expidus.org/show_bug.cgi?id=2939 */
    if(EXPIDUS_BACKDROP_IMAGE_TILED == istyle
       || EXPIDUS_BACKDROP_IMAGE_CENTERED == istyle)
    {
    ...
    final_image = expidus_backdrop_generate_canvas(backdrop);
...
static void
expidus_backdrop_file_ready_cb(GObject *source_object,
                            GAsyncResult *res,
                            gpointer user_data)
{
    ...
    g_input_stream_read_async(G_INPUT_STREAM(input_stream),
                              ...
                              EXPIDUS_BACKDROP_BUFFER_SIZE,
                              G_PRIORITY_LOW,
                              image_data->cancellable,
                              expidus_backdrop_file_input_stream_ready_cb,
                              image_data);
}

static void
expidus_backdrop_file_input_stream_ready_cb(GObject *source_object,
                                         GAsyncResult *res,
                                         gpointer user_data)
{
