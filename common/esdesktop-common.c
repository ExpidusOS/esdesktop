...
#include "expidus-backdrop.h" /* for ExpidusBackdropImageStyle */

/* Free the string whe done using it */
gchar*
esdesktop_get_monitor_name_from_gtk_widget(GtkWidget *widget, gint monitor_num)
{
...
gint
esdesktop_compare_paths(GFile *a, GFile *b)
{
    gchar *path_a, *path_b;
    gboolean ret;
...
gchar *
esdesktop_get_file_mimetype(const gchar *file)
{
    GFile *temp_file;
    GFileInfo *file_info;
...
gboolean
esdesktop_image_file_is_valid(const gchar *filename)
{
    ...
    file_mimetype = esdesktop_get_file_mimetype(filename);
...
gint
expidus_translate_image_styles(gint input)
{
    gint style = input;

    if(style <= 0 || style > EXPIDUS_BACKDROP_IMAGE_SPANNING_SCREENS)
        style = EXPIDUS_BACKDROP_IMAGE_STRETCHED;
...
/*
 * esdesktop_remove_whitspaces:
 * remove all whitespaces from string (not only trailing or leading)
 */
gchar*
esdesktop_remove_whitspaces(gchar* str)
{
    gchar* dest;
    guint offs, curr;
...
/* Adapted from markon_gtk_menu_create_menu_item because I don't want
 * to write it over and over.
 */
GtkWidget*
esdesktop_menu_create_menu_item_with_markup(const gchar *name,
                                            GtkWidget   *image)
{
...
GtkWidget*
esdesktop_menu_create_menu_item_with_mnemonic(const gchar *name,
                                              GtkWidget   *image)
{
...
void
esdesktop_get_screen_dimensions (GdkScreen *screen,
                                 gint      *width,
                                 gint      *height)
{
    gint x, y, w, h;
...
gint
esdesktop_get_monitor_num(GdkDisplay *display,
                          GdkMonitor *monitor)
{
    gint i;
...
gint
esdesktop_get_current_monitor_num(GdkDisplay *display)
{
    GdkSeat    *seat;
    GdkMonitor *monitor;
    gint        rootx, rooty;
    ...
    return esdesktop_get_monitor_num(display, monitor);
}
...
void
esdesktop_debug(const char *func, const char *file, int line, const char *format, ...)
{
    va_list args;
 ...
 */
void
esdesktop_debug_set(gboolean debug)
{
    enable_debug = debug;
    if(enable_debug)
